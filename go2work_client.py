...
class App(tk.Frame):
    ...
    def get_employees(self, employee_list):
        ...
        self.show_employee_frame.configure(background='NavajoWhite3')
        ...
        tk.Button(self.show_employee_frame, bg='light blue', text='Set Shift', default='active',
                  command=lambda: self.update_shift_or_delete_worker(1, received_data)).grid(row=1, column=0,
                                                                                             sticky='w',
                                                                                             pady=10, padx=0,
                                                                                             ipady=10,
                                                                                             ipadx=20)
        ...
        tk.Button(self.show_employee_frame, bg='light blue', text='Remove', default='active',
                  command=lambda: self.update_shift_or_delete_worker(2, received_data)).grid(row=1,
                                                                                             column=2, sticky='w',
                                                                                             pady=10, padx=0, ipady=10,
                                                                                             ipadx=20)
        tk.Button(self.show_employee_frame, bg='light blue', text='Update', default='active',
                  command=lambda: self.update_worker(received_data)).grid(row=1, column=3, sticky='w', ipady=10,
                                                                          ipadx=20)
    ...
    def update_worker(self, employees):
        ...
        tk.Label(window, text='Name', bg='NavajoWhite3', fg='snow', font=("Times New Roman", 16)).grid(row=0, column=0, sticky='w')
        tk.Label(window, text='Last Name', bg='NavajoWhite3', fg='snow', font=("Times New Roman", 16)).grid(row=1, column=0, sticky='w')
        tk.Label(window, text='Address', bg='NavajoWhite3', fg='snow', font=("Times New Roman", 16)).grid(row=2, column=0, sticky='w')
        tk.Label(window, text='Id', bg='NavajoWhite3', fg='snow', font=("Times New Roman", 16)).grid(row=3, column=0, sticky='w')
        tk.Label(window, text='Status', bg='NavajoWhite3', fg='snow', font=("Times New Roman", 16)).grid(row=4, column=0, sticky='w')
        tk.Button(window, text='Update',  bg='light blue', font=("Times New Roman", 10), command=lambda: make_update()).grid(row=5,
                                                                                                           column=0,
                                                                                                           sticky='w',
                                                                                                           padx=20,
                                                                                                           pady=20)
        tk.Button(window, text='Cancel', bg='light blue', font=("Times New Roman", 10), command=lambda: window.destroy()).grid(row=5,
                                                                                                              column=1,
                                                                                                              sticky='w',
                                                                                                              pady=20)

        def make_update():
            if self.add_status == '' or self.add_address == '' or self.add_last_name == '' or self.add_name == '' or self.add_id == '':
    ...
    def search_employee(self):
        ...
        self.search_frame.configure(background='NavajoWhite3')
        self.search_frame.pack(padx=50, pady=20, anchor='n')
        self.active_frame.append(self.search_frame)
        var = tk.IntVar()
        var.set(0)
        tk.Radiobutton(self.search_frame, text="By name", variable=var, value=1).grid(row=0, column=0,
                                                                                      sticky='w',
                                                                                      pady=10)
        tk.Radiobutton(self.search_frame, text="By id", variable=var, value=2).grid(row=1, column=0,
                                                                                    sticky='w',
                                                                                    pady=10)
        tk.Radiobutton(self.search_frame, text="By address", variable=var, value=3).grid(row=2, column=0,
                                                                                         sticky='w',
                                                                                         pady=10)
    ...
    def add_employee(self):
        ...
        self.add_id.grid(row=3, column=1, sticky='w', padx=20)
        tk.Label(self.add_employee_frame, bg='NavajoWhite3', fg='snow', text='Name', font=("Times New Roman", 16)).grid(row=0, column=0,
                                                                                          sticky='w')
        tk.Label(self.add_employee_frame, bg='NavajoWhite3', fg='snow', text='Last Name', font=("Times New Roman", 16)).grid(row=1, column=0,
                                                                                               sticky='w')
        tk.Label(self.add_employee_frame, bg='NavajoWhite3', fg='snow', text='Address', font=("Times New Roman", 16)).grid(row=2, column=0,
                                                                                             sticky='w')
        tk.Label(self.add_employee_frame, bg='NavajoWhite3', fg='snow', text='Id', font=("Times New Roman", 16)).grid(row=3, column=0,
                                                                                        sticky='w')
    ...
    def show_login_page(self):
        ...
        tk.Label(self.dialog_frame, text="Login to GO2WORK system", bg='NavajoWhite3', fg='snow', font=("Times New Roman", 22)).grid(row=0,
                                                                                                       columnspan=3,
                                                                                                       sticky='n',
                                                                                                       padx=20,
                                                                                                       pady=60)
        tk.Label(self.dialog_frame, text='Username:',bg='NavajoWhite3', fg='snow', font=("Times New Roman", 16)).grid(row=1, column=0,
                                                                                         sticky='w',
                                                                                         padx=20)
        self.user_input = tk.Entry(self.dialog_frame, background='white', width=24)
        self.user_input.grid(row=1, column=1, sticky='w', padx=20)
        self.user_input.focus_set()
        tk.Label(self.dialog_frame, text='Password:', bg='NavajoWhite3', fg='snow', font=("Times New Roman", 16)).grid(row=2, column=0,
                                                                                         sticky='w',
                                                                                         padx=20)
        self.pass_input.grid(row=2, column=1, sticky='w', padx=20)
        tk.Button(self.dialog_frame, text='OK', default='active', bg='light blue', command=lambda: self.click_ok()).grid(row=3,
                                                                                                        column=0,
                                                                                                        sticky='w',
                                                                                                        padx=20,
                                                                                                        pady=50,
                                                                                                        ipady=10,
                                                                                                        ipadx=20)
        tk.Button(self.dialog_frame, text='Cancel', default='active', bg='light blue', command=lambda: self.click_cancel()).grid(
                  row=3, column=1, sticky='w', padx=100, pady=50, ipady=10, ipadx=10)

    def show_route_page(self):
        self.forget_frames()
        self.route_frame = tk.Frame(self)
        self.route_frame.configure(background='NavajoWhite3')
