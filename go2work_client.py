...
class App(tk.Frame):
    ...
    def get_employees(self, employee_list):
        ...
        tk.Button(self.show_employee_frame, text = 'Unset Shift', default = 'active',
                  ...
                  column = 1, sticky = 'w', pady = 10, ipady = 10, ipadx = 20, padx = 60)
        tk.Button(self.show_employee_frame, text = 'Remove', default = 'active',
                  ...
                  column = 2, sticky = 'w', pady = 10, padx = 0, ipady = 10, ipadx = 20)
        tk.Button(self.show_employee_frame, text='Update', default='active',
                  command = lambda: self.update_worker(received_data)).grid(row=1, column=3, sticky='w',ipady = 10, ipadx = 20)
    ...
    def update_worker(self, employees):
        ...
        tk.Label(window, text = 'Name', font = ("Times New Roman", 14)).grid(row = 0, column = 0, sticky = 'w')
        tk.Label(window, text = 'Last Name', font = ("Times New Roman", 14)).grid(row = 1, column = 0, sticky = 'w')
        tk.Label(window, text = 'Address', font = ("Times New Roman", 14)).grid(row = 2, column = 0, sticky = 'w')
        tk.Label(window, text = 'Id', font = ("Times New Roman", 14)).grid(row = 3, column = 0, sticky = 'w')
        tk.Label(window, text = 'Status', font = ("Times New Roman", 14)).grid(row = 4, column = 0, sticky = 'w')
        tk.Button(window, text='Update', font=("Times New Roman", 10), command=lambda:make_update()).grid(row=5, column=0, sticky='w', padx=20, pady=20)
        tk.Button(window, text ='Cancel', font = ("Times New Roman", 10), command = lambda: window.destroy()).grid(row = 5, column = 1, sticky = 'w', pady = 20)
    ...
    def search_employee(self):
        ...
        tk.Button(self.search_frame, text = 'Search', font = ("Times New Roman", 10),
    ...
    def add_employee(self):
        ...
        self.add_id.grid(row = 3, column = 1, sticky = 'w', padx = 20)
        tk.Label(self.add_employee_frame, text = 'Name', font = ("Times New Roman", 14)).grid(row = 0, column = 0,
                                                                                              sticky = 'w')
        ...
        tk.Label(self.add_employee_frame, text = 'Address', font = ("Times New Roman", 14)).grid(row = 2, column = 0,
                                                                                                 sticky = 'w')
        tk.Label(self.add_employee_frame, text = 'Id', font = ("Times New Roman", 14)).grid(row = 3, column = 0,
                                                                                            sticky = 'w')
        tk.Button(self.add_employee_frame, text = 'Add', font = ("Times New Roman", 10),
        ...
        def send_to_db():
            ...
            if result:
                tkinter.messagebox.showinfo("Add Employee",
                                            "Employee {} {} successfully added.".format(add_data['firstname'], add_data['lastname']))
            else:
    ...
    def show_login_page(self):
        ...
        tk.Label(self.dialog_frame, text = "Login to GO2WORK system", font = ("Times New Roman", 16)).grid(row = 0,
                                                                                                           columnspan = 3,
                                                                                                           sticky = 'n',
                                                                                                           padx = 20,
                                                                                                           pady = 60)
        tk.Label(self.dialog_frame, text = 'Username:', font = ("Times New Roman", 14)).grid(row = 1, column = 0,
                                                                                             sticky = 'w',
                                                                                             padx = 20)
        self.user_input = tk.Entry(self.dialog_frame, background = 'white', width = 24)
        self.user_input.grid(row = 1, column = 1, sticky = 'w', padx = 20)
        self.user_input.focus_set()
        tk.Label(self.dialog_frame, text = 'Password:', font = ("Times New Roman", 14)).grid(row = 2, column = 0,
                                                                                             sticky = 'w',
                                                                                             padx = 20)
        self.pass_input.grid(row = 2, column = 1, sticky = 'w', padx = 20)
        tk.Button(self.dialog_frame, text = 'OK', default = 'active', command = lambda: self.click_ok()).grid(row = 3,
                                                                                                              column = 0,
                                                                                                              sticky = 'w',
                                                                                                              padx = 20,
                                                                                                              pady = 20,
                                                                                                              ipady = 10,
                                                                                                              ipadx = 20)
        tk.Button(self.dialog_frame, text = 'Cancel', default = 'active', command = lambda: self.click_cancel()).grid(
            row = 3,
            column = 1,
            sticky = 'w',
            padx = 20,
